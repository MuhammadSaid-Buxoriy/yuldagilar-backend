...
export class StatisticsService {
  ...
  static async getWeeklyDailyPoints(tg_id) {
    try {
      const progressHistory = await DatabaseService.getUserProgressHistory(tg_id, 7);
  ...
  static async getGlobalStatistics() {
    try {
      ...
      const totalPointsToday = todayStats?.reduce((sum, item) => sum + item.total_points, 0) || 0;

      return {
        totalUsers: totalUsers || 0,
        todayActiveUsers: todayActiveUsers || 0,
        totalPointsToday,
        averagePointsPerUser: todayActiveUsers > 0 ? Math.round(totalPointsToday / todayActiveUsers) : 0
      };
    } catch (error) {
